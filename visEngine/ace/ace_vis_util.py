...
class PointCloudBuffer:
    ...
    def update_buffer(self, pc_xyz, pc_clr, pc_errs=None):
        ...
        """
        if len(self.pc_xyz_buffer) != 0:
            return
        self.pc_xyz_buffer.append(pc_xyz)
        self.pc_clr_buffer.append(pc_clr)
        self.pc_err_buffer.append(pc_xyz[:, 0:1])

        print(f'xyz buffer: {len(self.pc_xyz_buffer)}')
        print(f'clr buffer: {len(self.pc_clr_buffer)}')
        print(f'err buffer: {len(self.pc_err_buffer)}')
...
class CameraTrajectoryBuffer:
    ...
    def __init__(self,
        ...
        self.trajectory_color = (104, 65, 233)
