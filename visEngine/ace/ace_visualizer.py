...
def create_camera(camera_info):
    ...
    """
    P_c2w=np.array(camera_info['P_c2w'])
    # P_c2w=np.linalg.inv(P_c2w)

    img_height = camera_info['height']
    img_width = camera_info['width']
    focal_x = fov2focal(camera_info['FovX'], img_width)
    focal_y = fov2focal(camera_info['FovY'], img_height)
    focal_avg = (focal_x + focal_y) / 2

    return [P_c2w, focal_avg], camera_info['image_path']
...
class ACEVisualizer:
    ...
    def __init__(self, target_path, flipped_portait, map_depth_filter, mapping_vis_error_threshold=10,
        ...
        self.pan_angle_coverage = 180  # degrees, opening angle of the camera pan
    ...
    def setup_mapping_visualisation_direct(self,
                                           pose_rgb_file,
                                           frame_count,
                                           camera_z_offset
                                           ):
        ...
        cam_list = []
        rgb_list = []
        camera_info_list = load_camera_info_from_json(pose_rgb_file)
        for camera_info in camera_info_list:
            cam_params, rgb_path = create_camera(camera_info)
            cam_list.append(cam_params)  # [4x4 matrix, focal]
            rgb_list.append(rgb_path)  # path

        print(f'successfully read cameras, count: {len(cam_list)}')
        # make sure poses and image files are in sync (relying on the file naming schema)

        # load mapping poses (4x4 matrices, camera to scene coordinates, OpenGL convention)
        mapping_poses = [self._convert_cv_to_gl(cam_params[0]) for cam_params in cam_list]
        ...
        # fill buffer with mapping trajectory
        self._generate_camera_trajectory(mapping_poses, rgb_list)
    ...
    def _render_pc(r, pc, camera, camera_pose):
        ...
        """
        scene = pyrender.Scene(bg_color=(255, 255, 255), ambient_light=(1, 1, 1))
        scene.add(pc)
    ...
    def _render_trajectory(r, trajectory, camera, camera_pose, frustum_images):
        ...
        """
        scene = pyrender.Scene(bg_color=(1, 1, 1, 1), ambient_light=(1, 1, 1))
    ...
    def render_mapping_frame_direct(self, pcd):
        """
        Update point cloud buffer with current scene coordinates and render frame.
        ...
        @param errors: N1 array of scalar reprojection errors for coloring the point cloud.
        """
        # scene_coordinates = pcd.points[0:256, :]  # N,3
        scene_coordinates = pcd.points # N,3
        # scene_coordinates_clr = pcd.colors[0:256, :]
        scene_coordinates_clr = pcd.colors
        # OpenCV to OpenGL convention
        scene_coordinates[:, 1] = -scene_coordinates[:, 1]
        scene_coordinates[:, 2] = -scene_coordinates[:, 2]
        # color point cloud according to errors
        ...
        # update rolling buffer
        self.point_cloud_buffer.update_buffer(scene_coordinates, scene_coordinates_clr, None)
